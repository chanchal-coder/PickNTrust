...
class TravelPicksBot {
  ...
  private async processUrl(url: string, messageText: string, msg: any): Promise<void> {
    try {
      // Process URL to get product information
      const productInfo = await urlProcessingService.processUrl(url);
      ...
      // Apply travel affiliate template
      const affiliateUrl = this.applyAffiliateTemplate(url);
      ...
      await this.saveToDatabase({
        name: productInfo.title || this.extractTitleFromMessage(messageText),
        description: productInfo.description || messageText.substring(0, 500),
        ...
        review_count: productInfo.reviewCount || 0,
