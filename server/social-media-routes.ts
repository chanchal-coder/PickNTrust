...
export function setupSocialMediaRoutes(app: Express) {
  ...
  app.post('/api/admin/social-media/post-pending', async (req, res) => {
    try {
      ...
      console.log('🚀 Manual trigger: Processing pending social media posts...');
      
      const result = await socialMediaPoster.processPendingPosts();
      
      console.log(`📊 Posting results: ${result.successful} successful, ${result.failed} failed out of ${result.processed} total`);
  ...
  app.post('/api/admin/social-media/test-credentials', async (req, res) => {
    try {
      ...
      if (result.success) {
        console.log(`✅ ${platform} credentials test successful`);
        res.json({
          success: true,
        ...
        console.log(`❌ ${platform} credentials test failed:`, result.results[0]?.error);
        res.json({
          success: false,
