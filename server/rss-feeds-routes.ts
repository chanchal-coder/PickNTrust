...
router.put('/api/admin/rss-feeds/:id', async (req, res) => {
  try {
    ...
    if (url) {
      ...
      const urlConflict = db.prepare('SELECT id FROM rss_feeds WHERE url = ? AND id != ?').get(url, id);
      if (urlConflict) {
        db.close();
        return res.status(400).json({ 
          success: false, 
...
router.patch('/api/admin/rss-feeds/:id/toggle', async (req, res) => {
  try {
    ...
    // Get current status
    const current = db.prepare('SELECT is_active FROM rss_feeds WHERE id = ?').get(id) as any;
    if (!current) {
      db.close();
      return res.status(404).json({ success: false, error: 'RSS feed not found' });
    }
    ...
    db.prepare('UPDATE rss_feeds SET is_active = ?, updated_at = ? WHERE id = ?')
