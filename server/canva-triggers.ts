...
export async function triggerCanvaForProduct(product: Product): Promise<void> {
  try {
    console.log(`🔄 Triggering Canva automation for product: ${product.name}`);
...
export async function triggerCanvaForBlog(blogPost: BlogPost): Promise<void> {
  try {
    console.log(`🔄 Triggering Canva automation for blog: ${blogPost.title}`);
...
export async function triggerCanvaForVideo(video: VideoContent): Promise<void> {
  try {
    console.log(`🔄 Triggering Canva automation for video: ${video.title}`);
...
export async function onContentCreated(
  ...
  try {
    ...
    console.log(`✅ Canva automation triggered successfully for ${contentType} #${contentId}`);
  } catch (error) {
    console.error(`❌ Failed to trigger Canva automation for ${contentType} #${contentId}:`, error);
  }
}
...
export async function triggerCanvaForBatch(
  ...
  console.log(`🔄 Triggering Canva automation for ${items.length} items...`);
  ...
  try {
    ...
    console.log(`✅ Batch Canva automation completed for ${items.length} items`);
  } catch (error) {
    console.error('❌ Batch Canva automation failed:', error);
  }
}
