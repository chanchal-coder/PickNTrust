...
class EnhancedTelegramManager {
   ...
   */
  private enhancedErrorHandler(error: Error, context: string, botName?: string): void {
    const timestamp = new Date().toISOString();
    const errorMessage = `[${timestamp}] ${context}${botName ? ` (${botName})` : ''}: ${error.message}`;
    
    console.error(`❌ ${errorMessage}`);
    
    // Log to file for debugging
    try {
      fs.appendFileSync('telegram-errors.log', errorMessage + '\n');
    } catch (logError) {
      console.error('Failed to write to error log:', logError.message);
    }
    
    // Update bot status if specific bot error
    if (botName && this.botStatuses.has(botName)) {
    ...
    if (error.message.includes('409')) {
      ...
      if (botName) {
        this.handleConflictRecovery(botName);
      }
    }
    ...
    if (error.message.includes('ETELEGRAM')) {
      console.log('📡 Telegram API issue - implementing backoff strategy');
      if (botName) {
        this.implementBackoffStrategy(botName);
      }
    }
  }
   ...
   */
  private async handleConflictRecovery(botName: string): Promise<void> {
    try {
      const status = this.botStatuses.get(botName);
      if (status) {
        status.conflictCount++;
        
        // If too many conflicts, disable bot temporarily
        if (status.conflictCount > 5) {
          console.log(`⚠️ ${botName}: Too many conflicts, disabling temporarily`);
          await this.disableBot(botName);
          
          // Re-enable after 5 minutes
          setTimeout(async () => {
            console.log(`🔄 ${botName}: Re-enabling after conflict cooldown`);
            await this.enableBot(botName);
          }, 5 * 60 * 1000);
        } else {
          // Restart bot
      ...
      console.error(`Failed to handle conflict recovery for ${botName}:`, error);
    }
  }

  /**
   * Implement backoff strategy for API errors
   */
  private implementBackoffStrategy(botName: string): void {
    const backoffTime = Math.min(1000 * Math.pow(2, this.recoveryAttempts.get(botName) || 0), 30000);
    
    console.log(`⏳ ${botName}: Implementing ${backoffTime}ms backoff`);
    
    setTimeout(() => {
      console.log(`🔄 ${botName}: Attempting recovery after backoff`);
      this.restartBot(botName);
    }, backoffTime);
  }

  /**
   * Robust posting function with retry logic
   */
  async robustPostToWebsite(productData: any, botName: string, retries: number = 3): Promise<any> {
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        console.log(`📤 ${botName}: Posting attempt ${attempt}/${retries}...`);
        
        // Get bot config to determine table
        const config = this.botConfigs.get(botName);
        if (!config) {
          throw new Error(`Bot configuration not found for ${botName}`);
        }
        
        // Insert product into appropriate table
        const tableName = config.tableName;
        const insertQuery = this.buildInsertQuery(tableName, productData);
        const result = this.db.prepare(insertQuery.sql).run(insertQuery.params);
        
        console.log(`✅ ${botName}: Successfully posted to website! Product ID: ${result.lastInsertRowid}`);
        
        // Update bot status
        const status = this.botStatuses.get(botName);
        if (status) {
          status.performance.dealsProcessed++;
          status.performance.successRate = Math.min(100, status.performance.successRate + 1);
        ...
        console.log(`❌ ${botName}: Attempt ${attempt} failed: ${error.message}`);
        
        if (attempt === retries) {
          this.enhancedErrorHandler(error as Error, 'Final posting attempt', botName);
          throw error;
        }
  ...
  /**
   * Build dynamic insert query for any product table
   */
  private buildInsertQuery(tableName: string, productData: any): { sql: string; params: any[] } {
    const fields = [
      'name', 'description', 'price', 'originalPrice', 'currency', 'imageUrl',
      'affiliateUrl', 'originalUrl', 'category', 'subcategory', 'rating',
      'reviewCount', 'discount', 'affiliateNetwork', 'telegramMessageId',
      'telegramChannelId', 'source', 'processingStatus'
    ];
    
    const validFields = fields.filter(field => productData[field] !== undefined);
    const placeholders = validFields.map(() => '?').join(', ');
    const fieldNames = validFields.join(', ');
    const params = validFields.map(field => productData[field]);
  ...
  async performHealthCheck(): Promise<boolean> {
    ...
    try {
      ...
      let healthyBots = 0;
      const totalBots = this.botConfigs.size;
      
      for (const [botName, config] of this.botConfigs.entries()) {
        const status = this.botStatuses.get(botName);
        
        if (status && status.status === 'active' && status.errorCount < 5) {
          healthyBots++;
        }
      }
      
      const healthPercentage = (healthyBots / totalBots) * 100;
      console.log(`📊 Bot Health: ${healthyBots}/${totalBots} bots healthy (${healthPercentage.toFixed(1)}%)`);
      
      // Check for recent errors
      const recentErrors = Array.from(this.botStatuses.values())
        .reduce((sum, status) => sum + status.errorCount, 0);
      
      console.log(`⚠️ Total Error Count: ${recentErrors}`);
      
      const isHealthy = healthPercentage >= 70 && recentErrors < 20;
      
      if (isHealthy) {
        console.log('✅ System health check passed!');
      } else {
        console.log('❌ System health check failed - review bot statuses');
      }
  ...
  async initializeBots(): Promise<void> {
    ...
    console.log('Launch Starting Enhanced Telegram Manager - 8-Bot System with Conflict Prevention');
    console.log('=' .repeat(80));
    console.log('AI Initializing 8 enabled bots with 409 conflict prevention...');
    console.log('');

    // Setup global cleanup for conflict-free bot manager
    conflictFreeBotManager.setupGlobalCleanup();

    // Register all bots with the conflict-free manager
    for (const [botName, config] of this.botConfigs) {
      if (config.isEnabled && config.botToken) {
        conflictFreeBotManager.registerBot({
          name: botName,
          ...
          shutdownFunction: async () => {
            await this.shutdownSingleBot(botName);
          }
        });
      }
    }
    ...
    await conflictFreeBotManager.initializeAllBots();
  ...
  private async initializeSingleBot(botName: string, config: BotConfig): Promise<void> {
    try {
      ...
      // Initialize the actual Telegram bot (now safely managed by Conflict-Free Manager)
      if (preferredMethod === 'telegram' && config.botToken && config.channelId) {
        await this.initializeActualTelegramBot(botName, config);
      }
    ...
    try {
      ...
      switch (botName) {
        case 'prime-picks':
          ...
          if (primePicksBot.primePicksBot) {
            await primePicksBot.primePicksBot.shutdown();
          }
          break;
        case 'cue-picks':
          const cuePicksBot = await import('./cue-picks-bot');
          if (cuePicksBot.cuePicksBot) {
            await cuePicksBot.cuePicksBot.shutdown();
          }
          break;
        case 'value-picks':
          const valuePicksBot = await import('./value-picks-bot');
          if (valuePicksBot.valuePicksBot) {
            await valuePicksBot.valuePicksBot.shutdown();
          }
          break;
        case 'lootbox':
          const lootBoxBot = await import('./loot-box-bot');
          if (lootBoxBot.lootBoxBot && lootBoxBot.lootBoxBot.stop) {
            await lootBoxBot.lootBoxBot.stop();
          }
          break;
        case 'click-picks':
          const clickPicksBot = await import('./click-picks-bot');
          if (clickPicksBot.clickPicksBot) {
            await clickPicksBot.clickPicksBot.shutdown();
          }
          break;
        default:
  ...
  async shutdown(): Promise<void> {
    ...
    // Shutdown all bots through conflict-free manager
    await conflictFreeBotManager.shutdownAllBots();
