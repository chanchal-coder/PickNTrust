...
class CanvaAutomationService {
  ...
  async triggerContentCreation(contentItem: ContentItem): Promise<void> {
    try {
      ...
      if (platforms.length === 0) {
        console.log('⚠️ No platforms enabled for automation');
        return;
      }

      console.log(`📱 Creating content for platforms: ${platforms.join(', ')}`);
      ...
      console.log('✅ Canva automation completed successfully');
    } catch (error) {
      console.error('❌ Canva automation failed:', error);
      throw error;
    }
  }
  ...
  private async createPlatformContent(
    ...
    try {
      ...
      if (!templateId) {
        console.log(`⚠️ No template available for ${platform}, skipping...`);
        return;
      }
      ...
      try {
        ...
        if (postResult.success) {
          console.log(`✅ Successfully posted to ${platform} for ${contentItem.type} ${contentItem.id}`);
        } else {
          console.log(`⚠️ Failed to post to ${platform} for ${contentItem.type} ${contentItem.id}:`, 
                     postResult.results[0]?.error || 'Unknown error');
        }
      } catch (postError) {
        console.log(`⚠️ Social media posting error for ${platform}:`, postError);
        // Don't fail the entire automation if posting fails
      }

      console.log(`✅ Content created for ${platform}`);
    } catch (error) {
  ...
  private selectTemplate(platform: string): string | null {
    try {
      ...
      if (defaultTemplate) {
        console.log(`🎯 Using default template for ${platform}`);
      ...
      console.log(`⚠️ No templates available for ${platform}`);
      return null;
    } catch (error) {
  ...
  private generateCaption(contentItem: ContentItem, settings: CanvaSettings): string {
    if (!settings.auto_generate_captions) {
      return settings.default_caption || 'Check out this amazing find! 🔥';
    }

    // Auto-generate based on content type
    switch (contentItem.type) {
      case 'product':
        return `🛍️ Discover ${contentItem.title}! ${contentItem.description.substring(0, 100)}... ${contentItem.price ? `Only $${contentItem.price}!` : ''} #ShopNow`;
      case 'service':
      ...
      case 'video':
        return `🎥 Watch: ${contentItem.title}! ${contentItem.description.substring(0, 100)}... Don't miss it! #Video`;
      default:
        return settings.default_caption || 'Check out this amazing content! 🔥';
    }
  }
  ...
  private saveCanvaPost(postData: {
    ...
    try {
      ...
      console.log('💾 Canva post saved to database');
    } catch (error) {
