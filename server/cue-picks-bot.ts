...
// Load cue-picks specific environment
import dotenv from 'dotenv';
import path from 'path';

// CRITICAL: Load bot-specific .env file FIRST
const cuepicksEnvPath = path.join(process.cwd(), '.env.cue-picks');
dotenv.config({ path: cuepicksEnvPath, override: true });

console.log('🔧 CUE-PICKS BOT: Loading environment from:', cuepicksEnvPath);
console.log('🔧 CUE-PICKS BOT TOKEN:', process.env.TELEGRAM_BOT_TOKEN?.substring(0, 10) + '...');
...
// Bot Configuration - Fixed to match .env.cue-picks format
const BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN;
const CHANNEL_ID = process.env.CHANNEL_ID;
const CHANNEL_USERNAME = process.env.CHANNEL_NAME;
const TARGET_PAGE = process.env.TARGET_PAGE || 'cue-picks';
const BOT_USERNAME = process.env.BOT_USERNAME || 'cuelinkspnt_bot';
const CHANNEL_NAME = process.env.CHANNEL_NAME || 'Cuelinks PNT';
...
if (!BOT_TOKEN || !CHANNEL_ID) {
  ...
  console.warn('Blog Please check TELEGRAM_BOT_TOKEN and CHANNEL_ID in .env.cue-picks');
...
// This bot MUST ONLY use .env.cue-picks
const REQUIRED_ENV_FILE = '.env.cue-picks';
const BOT_NAME = 'Cue Picks';
const EXPECTED_TOKEN_PREFIX = '8352384812';
...
class CuePicksBot {
  ...
  private async handleMessage(msg: TelegramBot.Message): Promise<void> {
    try {
      if (msg.chat.type === 'private') {
        ...
        console.log('Cue Picks: Private message received');
      } else if (msg.chat.id === parseInt(CHANNEL_ID!) || msg.chat.id.toString() === CHANNEL_ID) {
        console.log(`Cue Picks: Processing channel message from ${msg.chat.id}`);
        await this.processChannelMessage(msg);
      } else {
        console.log(`Cue Picks: Ignoring message from chat ${msg.chat.id} (expected ${CHANNEL_ID})`);
      }
    } catch (error) {
      console.error('Cue Picks Bot message handling error:', error);
    }
  }
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      webhookManager.registerBot('cue-picks', BOT_TOKEN, this.handleMessage.bind(this));
      
      // Webhook mode - no polling listeners needed
      console.log('📡 Cue Picks bot registered for webhook mode');
  ...
  private async saveProduct(productData: ProductData, message: TelegramBot.Message): Promise<void> {
    try {
      ...
      // Use direct SQL INSERT to save to cue_picks_products table
      const Database = require('better-sqlite3');
      const sqliteDb = new Database('./database.sqlite');
      
      const stmt = sqliteDb.prepare(`
        INSERT INTO cue_picks_products (
          ...
          source, telegramMessageId, processingStatus,
          createdAt, contentType, affiliateNetwork, telegramChannelId
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      ...
      sqliteDb.close();
      console.log('Success Cue Picks product saved to cue_picks_products table');
...
if (BOT_TOKEN && CHANNEL_ID) {
  console.log('Cue Picks Bot: Credentials found, ready for initialization');
} else {
  console.log('Cue Picks Bot disabled: Missing credentials');
}