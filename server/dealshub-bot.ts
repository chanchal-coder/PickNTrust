...
// This bot MUST ONLY use .env.deals-hub
const REQUIRED_ENV_FILE = '.env.deals-hub';
const BOT_NAME = 'DealsHub';
const EXPECTED_TOKEN_PREFIX = '8292764619';
...
// Load DealsHub environment configuration - Fixed to use correct .env file
const dealsHubEnvPath = path.join(process.cwd(), '.env.deals-hub');
...
if (fs.existsSync(dealsHubEnvPath)) {
  ...
  console.log('Warning .env.deals-hub file not found');
}

// Environment variables - Fixed to match .env.deals-hub format
const BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN;
const CHANNEL_ID = process.env.CHANNEL_ID;
const CHANNEL_USERNAME = process.env.CHANNEL_NAME;
const TARGET_PAGE = process.env.TARGET_PAGE || 'dealshub';
const BOT_USERNAME = process.env.BOT_USERNAME || 'dealshubpnt_bot';
const CHANNEL_NAME = process.env.CHANNEL_NAME || 'DealsHub Universal';
...
class DealsHubBot {
  ...
  private async handleMessage(msg: TelegramBot.Message): Promise<void> {
    try {
      if (msg.chat.type === 'private') {
        ...
        console.log('DealsHub: Private message received');
      } else if (msg.chat.id === parseInt(CHANNEL_ID!) || msg.chat.id.toString() === CHANNEL_ID) {
        console.log(`DealsHub: Processing channel message from ${msg.chat.id}`);
        await this.processChannelMessage(msg);
      } else {
        console.log(`DealsHub: Ignoring message from chat ${msg.chat.id} (expected ${CHANNEL_ID})`);
      }
    } catch (error) {
      console.error('DealsHub Bot message handling error:', error);
    }
  }
  ...
  async initialize() {
    try {
      ...
      webhookManager.registerBot('dealshub', BOT_TOKEN, this.handleMessage?.bind(this) || this.handleTelegramMessage?.bind(this));
      
      // Webhook mode - no polling listeners needed
      console.log('📡 DealsHub bot registered for webhook mode');
