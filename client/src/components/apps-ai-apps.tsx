                  ...
                  <div className="flex items-center justify-between pt-2">
                    <div className="flex items-center space-x-2">
                      ...
                      <div className="flex flex-col space-y-1">
                        ...
                        {(app as any).isFree || ((app as any).pricingType === 'free') ? (
                          <span className="text-lg font-bold text-green-600 dark:text-green-400">FREE</span>
                        ) : (app as any).pricingType === 'custom' ? (
                          <span className="text-lg font-bold text-green-600 dark:text-green-400">
                            {(app as any).priceDescription || "Custom Pricing"}
                          </span>
                        ) : app.price === '0' && !(app as any).monthlyPrice && !(app as any).yearlyPrice ? (
                          <span className="text-lg font-bold text-green-600 dark:text-green-400">FREE</span>
                        ) : (
                          <>
                            {/* Priority: Custom price description > Monthly > Yearly > Regular price */}
                            {(app as any).priceDescription && (app as any).priceDescription.trim() !== '' ? (
                              <span className="text-lg font-bold text-green-600 dark:text-green-400">{(app as any).priceDescription}</span>
                            ) : (app as any).monthlyPrice && (app as any).monthlyPrice !== '0' && (app as any).monthlyPrice !== 0 ? (
                              <div className="flex items-center space-x-2">
                                <span className="text-lg font-bold text-green-600 dark:text-green-400">{formatProductPrice((app as any).monthlyPrice, app.currency)}/month</span>
                                {app.originalPrice && (
                                  <span className="text-gray-500 line-through text-sm">{formatProductPrice(app.originalPrice, app.currency)}/month</span>
                                )}
                              </div>
                            ) : (app as any).yearlyPrice && (app as any).yearlyPrice !== '0' ? (
                              <div className="flex items-center space-x-2">
                                <span className="text-lg font-bold text-green-600 dark:text-green-400">{formatProductPrice((app as any).yearlyPrice, app.currency)}/year</span>
                                {app.originalPrice && (
                                  <span className="text-gray-500 line-through text-sm">{formatProductPrice(app.originalPrice, app.currency)}/year</span>
                                )}
                              </div>
                            ) : app.price && app.price !== '0' ? (
                              <div className="flex items-center space-x-2">
                                {/* For regular price, check pricingType to determine suffix */}
                                {(app as any).pricingType === 'monthly' ? (
                                  <span className="text-lg font-bold text-green-600 dark:text-green-400">{formatProductPrice(app.price, app.currency)}/month</span>
                                ) : (app as any).pricingType === 'yearly' ? (
                                  <span className="text-lg font-bold text-green-600 dark:text-green-400">{formatProductPrice(app.price, app.currency)}/year</span>
                                ) : (
                                  <span className="text-lg font-bold text-green-600 dark:text-green-400">{formatProductPrice(app.price, app.currency)}</span>
                                )}
                                {app.originalPrice && (
                                  <span className="text-gray-500 line-through text-sm">
                                    {formatProductPrice(app.originalPrice, app.currency)}
                                    {(app as any).pricingType === 'monthly' ? '/month' : (app as any).pricingType === 'yearly' ? '/year' : ''}
                                  </span>
                                )}
                              </div>
                            ) : (app as any).pricingType === 'yearly' ? (
                            ...
                            {(app as any).monthlyPrice && (app as any).yearlyPrice && (app as any).monthlyPrice !== '0' && (app as any).yearlyPrice !== '0' && !(app as any).priceDescription && (
                              <div className="text-xs text-gray-500">
                                <span>{formatProductPrice((app as any).monthlyPrice, app.currency)}/month or {formatProductPrice((app as any).yearlyPrice, app.currency)}/year</span>
                              </div>
                            )}
                            
                            {/* Show pricing type indicator for custom types */}
                            {(app as any).pricingType && !['monthly', 'yearly', 'one-time', 'free'].includes((app as any).pricingType) && (
                              <span className="text-xs text-gray-500 capitalize">({(app as any).pricingType})</span>
                            )}
                          </>
                        )}
                      </div>
                      ...
                      <div className="flex flex-col space-y-1">
                        ...
                        {(app as any).isFree || ((app as any).pricingType === 'free') ? (
                          ...
                          <>
                            {/* Priority: Custom price description > Monthly > Yearly > Regular price */}
                            {(app as any).priceDescription && (app as any).priceDescription.trim() !== '' ? (
                              <span className="text-sm font-bold text-green-600 dark:text-green-400">{(app as any).priceDescription}</span>
                            ) : (app as any).monthlyPrice && (app as any).monthlyPrice !== '0' && (app as any).monthlyPrice !== 0 ? (
                              <div className="flex items-center space-x-1">
                                <span className="text-sm font-bold text-green-600 dark:text-green-400">{formatProductPrice((app as any).monthlyPrice, app.currency)}/mo</span>
                                {app.originalPrice && (
                                  <span className="text-xs text-gray-500 line-through">{formatProductPrice(app.originalPrice, app.currency)}/mo</span>
                                )}
                              </div>
                            ) : (app as any).yearlyPrice && (app as any).yearlyPrice !== '0' ? (
                              <div className="flex items-center space-x-1">
                                <span className="text-sm font-bold text-green-600 dark:text-green-400">{formatProductPrice((app as any).yearlyPrice, app.currency)}/yr</span>
                                {app.originalPrice && (
                                  <span className="text-xs text-gray-500 line-through">{formatProductPrice(app.originalPrice, app.currency)}/yr</span>
                                )}
                              </div>
                            ) : app.price && app.price !== '0' ? (
                              <div className="flex items-center space-x-1">
                                {/* For regular price, check pricingType to determine suffix */}
                                {(app as any).pricingType === 'monthly' ? (
                                  <span className="text-sm font-bold text-green-600 dark:text-green-400">{formatProductPrice(app.price, app.currency)}/mo</span>
                                ) : (app as any).pricingType === 'yearly' ? (
                                  <span className="text-sm font-bold text-green-600 dark:text-green-400">{formatProductPrice(app.price, app.currency)}/yr</span>
                                ) : (
                                  <span className="text-sm font-bold text-green-600 dark:text-green-400">{formatProductPrice(app.price, app.currency)}</span>
                                )}
                                {app.originalPrice && (
                                  <span className="text-xs text-gray-500 line-through">
                                    {formatProductPrice(app.originalPrice, app.currency)}
                                    {(app as any).pricingType === 'monthly' ? '/mo' : (app as any).pricingType === 'yearly' ? '/yr' : ''}
                                  </span>
                                )}
                              </div>
                            ) : (app as any).pricingType === 'yearly' ? (
                            ...
                            {(app as any).monthlyPrice && (app as any).yearlyPrice && (app as any).monthlyPrice !== '0' && (app as any).yearlyPrice !== '0' && !(app as any).priceDescription && (
                              <div className="text-xs text-gray-500">
                                <span>{formatProductPrice((app as any).monthlyPrice, app.currency)}/mo or {formatProductPrice((app as any).yearlyPrice, app.currency)}/yr</span>
                              </div>
                            )}
