              ...
              >
                ...
                <div className="p-3 bg-white dark:bg-gray-800 text-gray-900 dark:text-white space-y-1.5 relative">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {(service as any).isFree || ((service as any).pricingType === 'free') || service.price === '0' || Number(service.price) === 0 ? (
                      <span className="text-lg font-bold text-green-600 dark:text-green-400">FREE</span>
                    ) : (service as any).priceDescription && (service as any).pricingType === 'custom' ? (
                      <span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">{(service as any).priceDescription}</span>
                    ) : ((service as any).pricingType === 'monthly' || (service as any).pricingType === 'Monthly Subscription') && (service as any).monthlyPrice && (service as any).monthlyPrice !== '0' ? (
                      <div className="flex items-center space-x-2">
                        <span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">{formatProductPrice((service as any).monthlyPrice, service.currency)}/month</span>
                        {service.originalPrice && (
                          <span className="text-gray-500 line-through text-sm">{formatProductPrice(service.originalPrice, service.currency)}/month</span>
                        )}
                      </div>
                    ) : ((service as any).pricingType === 'yearly' || (service as any).pricingType === 'Yearly Subscription') && (service as any).yearlyPrice && (service as any).yearlyPrice !== '0' ? (
                      <div className="flex items-center space-x-2">
                        <span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">{formatProductPrice((service as any).yearlyPrice, service.currency)}/year</span>
                        {service.originalPrice && (
                          <span className="text-gray-500 line-through text-sm">{formatProductPrice(service.originalPrice, service.currency)}/year</span>
                        )}
                      </div>
                    ) : (
                      <div className="flex items-center space-x-2">
                        {/* For regular price, check pricingType to determine suffix */}
                        {(service as any).pricingType === 'Monthly Subscription' ? (
                          <span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">{formatProductPrice(service.price, service.currency)}/month</span>
                        ) : (service as any).pricingType === 'Yearly Subscription' ? (
                          <span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">{formatProductPrice(service.price, service.currency)}/year</span>
                        ) : (service as any).pricingType === 'One-time Payment' ? (
                          <div className="flex flex-col">
                            <span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">{formatProductPrice(service.price, service.currency)}</span>
                        ...
                        {service.originalPrice && (service as any).pricingType !== 'One-time Payment' && (
                          <span className="text-gray-500 line-through text-sm">
                            {formatProductPrice(service.originalPrice, service.currency)}
                            {((service as any).pricingType === 'monthly' || (service as any).pricingType === 'Monthly Subscription') ? '/month' : ((service as any).pricingType === 'yearly' || (service as any).pricingType === 'Yearly Subscription') ? '/year' : ''}
                          </span>
                        )}
                      </div>
                    )}
