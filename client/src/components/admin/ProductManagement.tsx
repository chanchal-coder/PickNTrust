...
import { SUPPORTED_CURRENCIES, formatPrice, getCurrencySymbol } from '@/utils/currency';
...
export default function ProductManagement() {
    ...
    onSuccess: async (data) => {
      ...
      if (data && data.id) {
        try {
          ...
          console.log(`🎨 Triggering Canva automation for new ${contentType}:`, data.id);
          ...
          if (canvaResponse.ok) {
            ...
            console.log('✅ Canva automation triggered successfully:', canvaResult);
            toast({
              title: '🎨 Automation Triggered!',
              description: 'Product added and Canva automation started for social media posting.',
            });
          } else {
            console.log('⚠️ Canva automation failed, but product was added successfully');
            toast({
              title: 'Product Added ✅',
          ...
          console.log('⚠️ Canva automation error:', canvaError);
          toast({
            title: 'Product Added ✅',
  ...
  const handleAddProduct = (e: React.FormEvent) => {
    ...
    if (activeTab === 'services' || activeTab === 'aiapps') {
      ...
      if (!newProduct.isFree) {
        ...
        if (needsPrice && !newProduct.price) {
          toast({
            title: 'Validation Error',
        ...
        if (needsMonthlyPrice && !newProduct.monthlyPrice) {
          toast({
            title: 'Validation Error',
        ...
        if (needsYearlyPrice && !newProduct.yearlyPrice) {
          toast({
            title: 'Validation Error',
  ...
  const handleShare = (platform: string, product: Product) => {
    ...
    switch (platform) {
      ...
      case 'whatsapp':
        ...
        break;
      case 'instagram':
        const instagramText = `🛍️ Amazing Deal Alert! ${product.name}\n\n💰 Price: ${formatPrice(product.price, product.currency || 'INR')}${product.originalPrice ? ` (was ${formatPrice(product.originalPrice, product.currency || 'INR')})` : ''}\n\n✨ Shop now at PickNTrust\n\n#PickNTrust #Deals #${product.category.replace(/\s+/g, '')} #Shopping`;
  ...
  const handleEditProduct = (product: Product) => {
    ...
    setNewProduct({
      ...
      timerDuration: product.timerDuration?.toString() || '24',
            ...
            <form onSubmit={handleAddProduct} className="space-y-4">
              ...
              {(activeTab === 'services' || activeTab === 'aiapps') ? (
                <div className="space-y-4">
                  ...
                  {/* Currency Selection for Services */}
                  <div>
                  ...
                  {!newProduct.isFree && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {(newProduct.pricingType === 'one-time' || newProduct.pricingType === 'custom') && (
                        <div>
                          <label className="block text-sm font-medium mb-2 text-blue-300">
                            Price ({getCurrencySymbol(newProduct.currency)})
                          </label>
                          <input
                            type="text"
                      ...
                      {(newProduct.pricingType === 'monthly' || newProduct.pricingType === 'custom') && (
                        <div>
                          <label className="block text-sm font-medium mb-2 text-blue-300">
                            Monthly Price ({getCurrencySymbol(newProduct.currency)})
                          </label>
                          <input
                            type="text"
                      ...
                      {(newProduct.pricingType === 'yearly' || newProduct.pricingType === 'custom') && (
                        <div>
                          <label className="block text-sm font-medium mb-2 text-blue-300">
                            Yearly Price ({getCurrencySymbol(newProduct.currency)})
                          </label>
                          <input
                            type="text"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium mb-2 text-blue-300">
                        Original Price ({getCurrencySymbol(newProduct.currency)})
                      </label>
                      <input
                        type="text"
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center">
                  <input
                    ...
                    className="mr-2"
                  />
                  <label htmlFor="isService" className="text-sm font-medium text-blue-300">💳 Service Product (Cards & Services)</label>
                </div>
                ...
                <div className="flex items-center">
                  <input
                    ...
                    className="mr-2"
                  />
                  <label htmlFor="isAIApp" className="text-sm font-medium text-blue-300">🤖 Apps & AI Apps</label>
                </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      <span className="text-green-400 font-bold">{formatPrice(product.price, product.currency || 'INR')}</span>
                      {product.originalPrice && (
                        <span className="text-gray-500 line-through">{formatPrice(product.originalPrice, product.currency || 'INR')}</span>
                      )}
