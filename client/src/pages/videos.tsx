...
import { Play, Clock, Calendar, Tag, Search, Filter, Grid, List, Trash2, ExternalLink } from 'lucide-react';
                      ...
                      <SelectContent className="rounded-xl border-0 shadow-2xl bg-white/95 dark:bg-slate-900/95 backdrop-blur-md">
                        ...
                        {platforms.map((platform) => (
                          <SelectItem key={platform} value={platform} className="rounded-lg">
                            {platform === 'youtube' ? '<i className="fas fa-tv"></i>' : platform === 'vimeo' ? '<i className="fas fa-video"></i>' : '<i className="fas fa-film"></i>'} {platform}
                          </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                        ...
                        <div className={viewMode === 'grid' ? 'relative' : 'md:flex relative'}>
                          ...
                          <div className={`relative overflow-hidden ${viewMode === 'grid' ? 'aspect-video rounded-t-3xl' : 'md:w-80 aspect-video md:aspect-square md:rounded-l-3xl md:rounded-tr-none rounded-t-3xl'}`}>
                            ...
                            >
                              ...
                              />
                              {/* Play button overlay */}
                              <div className="absolute inset-0 bg-black/30 flex items-center justify-center opacity-0 group-hover/thumb:opacity-100 transition-all duration-300">
                                <div className="bg-red-600 rounded-full p-4 transform scale-75 group-hover/thumb:scale-100 transition-transform duration-300 shadow-2xl">
                              ...
                              <div className="flex items-start justify-between gap-3">
                                ...
                                <div className="shrink-0">
                                  <Badge className={`px-3 py-1.5 rounded-full font-semibold text-xs shadow-lg border-0 ${
                                    ...
                                    {video.platform.toLowerCase() === 'youtube' ? '<i className="fas fa-tv"></i>' : video.platform.toLowerCase() === 'vimeo' ? '<i className="fas fa-video"></i>' : '<i className="fas fa-film"></i>'} {video.platform}
                                  </Badge>
                                </div>
                              </div>
