...
// Deals Hub page uses real data from the database via API endpoints
...
export default function DealsHub() {
  ...
  const handleBulkDelete = async (deleteAll = false) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/products/page/deals-hub'] });
      queryClient.invalidateQueries({ queryKey: ['/api/deals-hub'] });
  ...
  // Fetch products from dedicated Deals Hub database table
  const { data: allDealsProducts = [], isLoading: productsLoading, error: productsError } = useQuery<Product[]>({
    queryKey: ['/api/products/page/deals-hub', selectedCategory],
    queryFn: async (): Promise<Product[]> => {
      const url = selectedCategory 
        ? `/api/products/page/deals-hub?category=${encodeURIComponent(selectedCategory)}`
        : '/api/products/page/deals-hub';
      
      const response = await fetch(url);
      if (!response.ok) {
        throw new Error('Failed to fetch Deals Hub products');
      }
  ...
  // Get unique categories from the actual products for sidebar
  const availableCategories = Array.from(new Set(
    allDealsProducts.map(product => product.category).filter(Boolean)
  ));

  // Derive available genders from loaded products (aggregate Kids when boys/girls present)
  const availableGenders = useMemo(() => {
    const set = new Set<string>();
    (allDealsProducts as Product[]).forEach((p) => {
  ...
  // Fetch deal types for additional filtering
  const { data: availableDealTypes = [] } = useQuery<string[]>({
    queryKey: ['/api/deal-types/deals-hub'],
    queryFn: async (): Promise<string[]> => {
      const response = await fetch('/api/deal-types/deals-hub');
      if (!response.ok) {
        return ['flash', 'daily', 'weekly', 'clearance', 'general'];
      }
      return response.json();
    },
  });
  ...
  const handleAffiliateClick = async (productId: string | number, networkId?: number, affiliateUrl?: string) => {
    try {
      await fetch('/api/affiliate/track-click', {
        ...
        body: JSON.stringify({
          productId,
          tableName: 'deals_hub_products', // Dedicated Deals Hub table
  ...
  const filteredProducts = allDealsProducts.filter((product: any) => {
  ...
  const handleGenderChange = (g: string) => {
    ...
    const basePath = typeof window !== 'undefined' ? window.location.pathname : '/deals-hub';
  ...
  useEffect(() => {
    if (productsError) {
      toast({
        ...
        description: "Failed to load Deals Hub products. Please try again.",
        variant: "destructive",
      });
    }
  }, [productsError, toast]);
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
                    <i className="fas fa-fire"></i> Hot Deals ({filteredProducts.length})
                  </h2>
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400">
                  Showing {filteredProducts.length} of {allDealsProducts.length} exclusive deals
                </div>
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
                Flash deals, daily offers, and exclusive discounts - sorted by priority and discount
              </p>
            </div>
            ...
            {productsLoading ? (
              <div className="text-center py-16">
                ...
                <p className="text-gray-500 dark:text-gray-500">
                  Loading flash deals, daily offers, and exclusive discounts.
                </p>
              </div>
            ) : filteredProducts.length === 0 ? (
              <div className="text-center py-16">
                <div className="text-6xl mb-4"><i className="fas fa-fire"></i></div>
                <h3 className="text-xl font-semibold text-gray-600 dark:text-gray-400 mb-2">
                  {allDealsProducts.length === 0 ? 'No deals available' : 'No deals found'}
                </h3>
                <p className="text-gray-500 dark:text-gray-500">
                  {allDealsProducts.length === 0 
                    ? 'Flash deals and exclusive offers will appear here when posted to Deals Hub channel.' 
                  ...
                  <div key={product.id} className="relative">
                    ...
                    {bulkDeleteMode && isAdmin && (
                      <div className="absolute top-2 right-2 z-20">
                        <input
                          type="checkbox"
                          checked={selectedProducts.includes(`dealshub_${product.id}`)}
                          onChange={(e) => {
                            const productId = `dealshub_${product.id}`;
                            if (e.target.checked) {
                    ...
                    <AmazonProductCard 
                       key={`dealshub_${product.id}`} 
                       product={{
                       id: `dealshub_${product.id}`,
                       name: product.name || '',
                       ...
                       affiliate_network: product.affiliate_network || 'DealsHub',
                       networkBadge: product.deal_badge || 'Hot Deal',
                       affiliateNetwork: 'DealsHub Network',
                       sourceType: 'dealshub',
                       source: 'deals-hub',
                       displayPages: ['deals-hub']
                     }}
