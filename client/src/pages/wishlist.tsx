                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="flex justify-between items-center mb-4">
                      <div>
                        ...
                        {(product as any).isFree || (product.pricingType === 'free') ? (
                          <span className="text-2xl font-bold text-green-600">FREE</span>
                        ) : (product as any).priceDescription ? (
                          <span className="text-2xl font-bold text-accent-green">{(product as any).priceDescription}</span>
                        ) : product.monthlyPrice && product.monthlyPrice !== '0' ? (
                          <div>
                            <span className="text-2xl font-bold text-accent-green">{formatPrice(product.monthlyPrice, product.currency || 'INR')}/month</span>
                            {product.originalPrice && product.originalPrice > product.price && (
                              <span className="text-sm text-gray-500 line-through ml-2">
                                {formatPrice(product.originalPrice, product.currency || 'INR')}/month
                              </span>
                            )}
                          </div>
                        ) : product.yearlyPrice && product.yearlyPrice !== '0' ? (
                          <div>
                            <span className="text-2xl font-bold text-accent-green">{formatPrice(product.yearlyPrice, product.currency || 'INR')}/year</span>
                            {product.originalPrice && product.originalPrice > product.price && (
                              <span className="text-sm text-gray-500 line-through ml-2">
                                {formatPrice(product.originalPrice, product.currency || 'INR')}/year
                              </span>
                            )}
                          </div>
                        ) : product.pricingType === 'monthly' ? (
                          <div>
                            <span className="text-2xl font-bold text-accent-green">{formatPrice(product.price, product.currency || 'INR')}/month</span>
                            {product.originalPrice && product.originalPrice > product.price && (
                              <span className="text-sm text-gray-500 line-through ml-2">
                                {formatPrice(product.originalPrice, product.currency || 'INR')}/month
                              </span>
                        ...
                        ) : product.pricingType === 'yearly' ? (
                          <div>
                            <span className="text-2xl font-bold text-accent-green">{formatPrice(product.price, product.currency || 'INR')}/year</span>
                            {product.originalPrice && product.originalPrice > product.price && (
                              <span className="text-sm text-gray-500 line-through ml-2">
                                {formatPrice(product.originalPrice, product.currency || 'INR')}/year
                              </span>
