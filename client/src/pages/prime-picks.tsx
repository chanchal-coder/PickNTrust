...
// Prime Picks page now uses real data from the database via API endpoints
...
export default function PrimePicks() {
  ...
  // Fetch products for Prime Picks page
  const { data: allPrimeProducts = [], isLoading: productsLoading, error: productsError } = useQuery<Product[]>({
    queryKey: ['/api/products/page/prime-picks', selectedCategory],
    queryFn: async (): Promise<Product[]> => {
      const url = selectedCategory 
        ? `/api/products/page/prime-picks?category=${encodeURIComponent(selectedCategory)}`
        : '/api/products/page/prime-picks';
      
      const response = await fetch(url);
      if (!response.ok) {
        throw new Error('Failed to fetch Prime Picks products');
      }
  ...
  // Fetch categories for Prime Picks page
  const { data: availableCategories = [] } = useQuery<string[]>({
    queryKey: ['/api/categories/page/prime-picks'],
    queryFn: async (): Promise<string[]> => {
      const response = await fetch('/api/categories/page/prime-picks');
      if (!response.ok) {
        throw new Error('Failed to fetch Prime Picks categories');
      }
      return response.json();
    },
    staleTime: 0, // Real-time updates
  ...
  useEffect(() => {
    if (productsError) {
      toast({
        ...
        description: "Failed to load Prime Picks products. Please try again.",
        variant: "destructive",
      });
    }
  }, [productsError, toast]);
  ...
  const handleBulkDelete = async (deleteAll = false) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/products/page/prime-picks'] });
  ...
  return (
    <UniversalPageLayout pageId="prime-picks" enableContentOverlays={false} enableFloatingOverlays={false}>
      ...
      {/* Prime Picks Videos Section - Only shows if videos exist for prime-picks */}
      <PageVideosSection 
        page="prime-picks" 
