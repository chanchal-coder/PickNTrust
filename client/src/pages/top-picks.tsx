                     ...
                     <div>
                       ...
                       {(product as any).isFree || ((product as any).pricingType === 'free') ? (
                         ...
                         <div className="flex items-center space-x-2">
                           <span className="text-2xl font-bold text-navy dark:text-blue-400">{formatProductPrice((product as any).monthlyPrice, product.currency)}/month</span>
                           {product.originalPrice && (
                             <span className="text-gray-400 dark:text-gray-500 line-through ml-2 text-base">{formatProductPrice(product.originalPrice, product.currency)}/month</span>
                           )}
                         </div>
                       ) : (product as any).yearlyPrice && (product as any).yearlyPrice !== '0' ? (
                         <div className="flex items-center space-x-2">
                           <span className="text-2xl font-bold text-navy dark:text-blue-400">{formatProductPrice((product as any).yearlyPrice, product.currency)}/year</span>
                           {product.originalPrice && (
                             <span className="text-gray-400 dark:text-gray-500 line-through ml-2 text-base">{formatProductPrice(product.originalPrice, product.currency)}/year</span>
                           )}
                         </div>
                       ) : (
                         <div className="flex items-center space-x-2">
                           ...
                           {(product as any).pricingType === 'monthly' ? (
                             <span className="text-2xl font-bold text-navy dark:text-blue-400">{formatProductPrice(product.price, product.currency)}/month</span>
                           ) : (product as any).pricingType === 'yearly' ? (
                             <span className="text-2xl font-bold text-navy dark:text-blue-400">{formatProductPrice(product.price, product.currency)}/year</span>
                           ) : (
                             <span className="text-2xl font-bold text-navy dark:text-blue-400">{formatProductPrice(product.price, product.currency)}</span>
                           )}
                           {product.originalPrice && (
                             <span className="text-gray-400 dark:text-gray-500 line-through ml-2 text-base">
