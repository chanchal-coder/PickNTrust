            ...
            {searchResults.map((product: any) => (
              <div key={product.id} className="bg-white rounded-lg shadow p-6">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-2xl font-bold text-green-600">{formatPrice(product.price, (product as any).currency || 'INR')}</span>
                {product.originalPrice && (
                  <span className="text-sm text-gray-500 line-through">{formatPrice(product.originalPrice, (product as any).currency || 'INR')}</span>
                  )}
                </div>
              </div>
            ))}
