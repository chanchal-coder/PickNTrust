...
async function addCategoryTypeFields() {
  console.log('Refresh Adding category type fields to database...');
  ...
  try {
    ...
    if (hasIsForProducts && hasIsForServices) {
      console.log('Success Category type fields already exist');
      return;
    }
    ...
    console.log('Refresh Setting default values for existing categories...');
    db.exec(`
      UPDATE categories 
    ...
    console.log('Success Successfully added category type fields');
    ...
    console.log(`\nStats Current categories (${categories.length} total):`);
    categories.forEach(cat => {
      console.log(`  - ${cat.name}: Products=${cat.is_for_products}, Services=${cat.is_for_services}`);
    });
    
  } catch (error) {
    console.error('Error Error adding category type fields:', error);
    throw error;
  } finally {
    db.close();
  }
