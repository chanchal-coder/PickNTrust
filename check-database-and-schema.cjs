...
console.log('Search CHECKING DATABASE AND SCHEMA...\n');

try {
  // 1. Check current database structure
  console.log('Upload Step 1: Checking current database structure...');
  ...
  console.log(`\n🎨 canva_settings table exists: ${canvaSettingsExists ? 'Success YES' : 'Error NO'}`);

  if (canvaSettingsExists) {
    console.log('\nStats canva_settings table structure:');
  ...
  console.log('\nUpload Step 2: Checking schema files...');
  
  const schemaPath = path.join(process.cwd(), 'shared', 'sqlite-schema.ts');
  if (fs.existsSync(schemaPath)) {
    console.log('Success Found shared/sqlite-schema.ts');
    const schemaContent = fs.readFileSync(schemaPath, 'utf8');
    
    if (schemaContent.includes('canvaSettings')) {
      console.log('Success canvaSettings table defined in schema');
      
      // Check if it has the new fields
      if (schemaContent.includes('defaultCaption') && schemaContent.includes('defaultHashtags')) {
        console.log('Success Manual caption/hashtag fields found in schema');
      } else {
        console.log('Error Manual caption/hashtag fields MISSING from schema');
        console.log('🔧 This is likely the root cause of the issue!');
      }
    } else {
      console.log('Error canvaSettings table NOT defined in schema');
      console.log('🔧 This is the root cause of the issue!');
    }
  } else {
    console.log('Error shared/sqlite-schema.ts not found');
  }

  // 3. Check storage.ts
  const storagePath = path.join(process.cwd(), 'server', 'storage.ts');
  if (fs.existsSync(storagePath)) {
    console.log('Success Found server/storage.ts');
    const storageContent = fs.readFileSync(storagePath, 'utf8');
    
    if (storageContent.includes('updateCanvaSettings')) {
      console.log('Success updateCanvaSettings method found');
      
      if (storageContent.includes('default_caption') && storageContent.includes('default_hashtags')) {
        console.log('Success Manual caption/hashtag fields handled in storage');
      } else {
        console.log('Error Manual caption/hashtag fields NOT handled in storage');
      }
    } else {
      console.log('Error updateCanvaSettings method not found');
    }
  } else {
    console.log('Error server/storage.ts not found');
  }
  ...
  console.error('Error Error checking database and schema:', error);
  process.exit(1);
}
