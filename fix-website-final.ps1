# Fix Website Final - Complete EC2 Server Fix
...
Write-Host "=== FIXING WEBSITE - FINAL ATTEMPT ===" -ForegroundColor Green
...
# Step 6: Update Nginx configuration to ensure it's correct
...
Write-Host "`n8. Checking Nginx status..." -ForegroundColor Yellow
ssh -i $KEY_PATH $EC2_USER@$EC2_IP "sudo systemctl status nginx --no-pager"

# Step 9: Wait a moment for everything to settle
Write-Host "`n9. Waiting for services to settle..." -ForegroundColor Yellow
Start-Sleep -Seconds 5

# Step 10: Test website accessibility multiple times
Write-Host "`n10. Testing website accessibility..." -ForegroundColor Yellow
for ($i = 1; $i -le 3; $i++) {
    Write-Host "Test attempt $i..." -ForegroundColor Cyan
    try {
...
Write-Host "`n11. Testing API endpoint..." -ForegroundColor Yellow
try {
...
Write-Host "`n12. Final status check..." -ForegroundColor Yellow
ssh -i $KEY_PATH $EC2_USER@$EC2_IP "pm2 status && echo '--- Nginx processes ---' && ps aux | grep nginx | grep -v grep"
