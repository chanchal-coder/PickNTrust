    ...
    app.get("/api/services", async (req, res) => {
        ...
        WHERE (
          ...
          OR content_type = 'service'
            ...
            const services = sqliteDb.prepare(servicesQuery).all();
    ...
    app.get("/api/products/page/:page", async (req, res) => {
        try {
            ...
            if ((page || '').toLowerCase() === 'top-picks') {
                // Keep Top Picks concise regardless of client-provided limit
                const effectiveLimit = Math.min(parsedLimit, 12);
                const ucBindings = {
        ...
        `;
          ...
          `;
                ...
                console.log(`Found ${products.length} unified_content products for "top-picks" (capped)`);
                ...
                try {
                    ...
                    if ((page || '').toLowerCase() === 'loot-box') {
                        try {
                            ...
                            const isInternalGo = (u) => !!u && /https?:\/\/[^\/]*pickntrust\.com\/go\//i.test(String(u));
      ...
      `).get(parent);
        ...
        `;
            ...
            const subcatFilters = (() => {
                const clauses = [];
                if (UC_COLS.status)
                    clauses.push(`(uc.status = 'active' OR uc.status = 'published' OR uc.status IS NULL)`);
                if (UC_COLS.visibility)
                    clauses.push(`(uc.visibility IN ('public','visible') OR uc.visibility IS NULL)`);
                if (UC_COLS.processing_status)
                    clauses.push(`(uc.processing_status = 'completed' OR uc.processing_status = 'active' OR uc.processing_status IS NULL)`);
                return clauses.length ? ` AND ${clauses.join(' AND ')}` : '';
            })();
