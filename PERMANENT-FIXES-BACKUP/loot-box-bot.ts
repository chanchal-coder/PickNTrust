...
// Loot Box Bot Configuration
const BOT_TOKEN = process.env.LOOT_BOX_BOT_TOKEN;
const CHANNEL_ID = process.env.LOOT_BOX_CHANNEL_ID;
const CHANNEL_USERNAME = process.env.LOOT_BOX_CHANNEL_USERNAME;
const TARGET_PAGE = process.env.LOOT_BOX_TARGET_PAGE || 'loot-box';
const AFFILIATE_TAG = process.env.LOOT_BOX_AFFILIATE_TAG || 'ref=lootbox';
...
if (BOT_TOKEN && CHANNEL_ID && !process.env.ENHANCED_MANAGER_ACTIVE) {
  lootBoxBot.initialize().catch(console.error);
} else if (!BOT_TOKEN || !CHANNEL_ID) {
  console.log('Warning Loot Box automation disabled - missing credentials');
}
