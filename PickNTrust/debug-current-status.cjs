console.log('🔍 Debugging current server and API status...');
...
async function testVideoAdding() {
  ...
  try {
    ...
    console.log('❌ Video adding failed:', error.message);
    return false;
  }
}
...
async function runDiagnostics() {
  ...
  console.log('\n=== DIAGNOSTIC RESULTS ===');
  console.log(`Server Status: ${serverOk ? '✅ RUNNING' : '❌ DOWN'}`);
  console.log(`Product Adding: ${productOk ? '✅ WORKING' : '❌ FAILING'}`);
  console.log(`Video Adding: ${videoOk ? '✅ WORKING' : '❌ FAILING'}`);
