...
console.log('🚨 FIXING ALL ERRORS AT ONCE...\n');

try {
  // 1. Fix the production database
  console.log('📁 Step 1: Fixing production database...');
  ...
  `);
  console.log('✅ canva_settings table created with manual fields');
  ...
  if (existingSettings.count === 0) {
    console.log('📝 Inserting default settings with manual caption/hashtag templates...');
    db.prepare(`
    ...
    console.log('🔄 Updating existing settings to add manual caption/hashtag fields...');
    try {
      db.exec(`
      ...
      db.prepare(`
        ...
        SET 
          default_caption = COALESCE(default_caption, '🛍️ Amazing {category} Alert! ✨ {title} 💰 Price: ₹{price} 🔗 Get the best deals at PickNTrust!'),
          default_hashtags = COALESCE(default_hashtags, '#PickNTrust #Deals #Shopping #BestPrice #Sale #Discount #OnlineShopping #India')
        WHERE id = 1
      `).run();
      console.log('✅ Existing settings updated with manual fields');
    } catch (alterError) {
  ...
  console.log('📁 Step 2: Updating storage.ts to handle manual caption/hashtag fields...');
  ...
  if (fs.existsSync(storagePath)) {
    ...
    stmt.run(
      ...
      if (oldMethod.test(storageContent)) {
        ...
        console.log('✅ storage.ts updated with manual caption/hashtag support');
      } else {
    ...
    console.log('⚠️ storage.ts not found, skipping update');
  }

  console.log('\n🎉 ALL ERRORS FIXED SUCCESSFULLY!');
  console.log('\n📋 What was fixed:');
  console.log('  ✅ Created missing canva_settings table');
  console.log('  ✅ Added default_caption and default_hashtags columns');
  console.log('  ✅ Inserted default settings with manual templates');
  console.log('  ✅ Updated storage.ts to handle new fields');
  console.log('  ✅ Frontend already updated to send new fields');
  ...
  console.log('\n✨ Manual Caption/Hashtag Fields Now Available:');
...
VALUES (1, 0, 1, 1, '🛍️ Amazing {category} Alert! ✨ {title} 💰 Price: ₹{price} 🔗 Get the best deals at PickNTrust!', '#PickNTrust #Deals #Shopping #BestPrice #Sale #Discount #OnlineShopping #India', '["instagram","facebook"]', 'immediate', 0);
  `);
