...
export default function CategoryPage() {
  ...
  const addManualProduct = async () => {
    // Enhanced validation for services vs products
    if (!manualProduct.name || !manualProduct.affiliateUrl) {
      toast({
        title: 'Missing Required Fields',
        description: `Please fill in ${activeProductTab === 'services' ? 'service' : 'product'} name and URL.`,
        variant: 'destructive',
      });
      return;
    }

    // Price validation - different logic for services vs products
    if (activeProductTab === 'services') {
    ...
    try {
      ...
      const payload = {
        ...
        // Service-specific fields - only include if it's a service
        pricingType: activeProductTab === 'services' ? (manualProduct.pricingType || 'one-time') : null,
        monthlyPrice: activeProductTab === 'services' && manualProduct.monthlyPrice ? parseFloat(manualProduct.monthlyPrice.toString().replace(/[^\d.]/g, '')) : null,
        yearlyPrice: activeProductTab === 'services' && manualProduct.yearlyPrice ? parseFloat(manualProduct.yearlyPrice.toString().replace(/[^\d.]/g, '')) : null,
        isFree: activeProductTab === 'services' ? Boolean(manualProduct.isFree) : false,
        priceDescription: activeProductTab === 'services' ? (manualProduct.priceDescription?.trim() || null) : null,
      };
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to add ${activeProductTab === 'services' ? 'service' : 'product'}: ${errorText}`);
      }

      toast({
        title: `${activeProductTab === 'services' ? 'Service' : 'Product'} Added!`,
        description: `Manual ${activeProductTab === 'services' ? 'service' : 'product'} has been added to the catalog successfully.`,
      });
                    ...
                    <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
                      <DialogHeader>
                        <DialogTitle className="text-2xl text-navy dark:text-blue-400">
                          Add {activeProductTab === 'services' ? 'Service' : 'Product'} to {categoryInfo.title}
                        </DialogTitle>
                        <DialogDescription>
                          Add a new {activeProductTab === 'services' ? 'service' : 'product'} by extracting details from a URL or entering manually
                        </DialogDescription>
                      </DialogHeader>
                      ...
                      <div className="space-y-6">
                        ...
                        <Card>
                          <CardHeader>
                            ...
                            <CardDescription>
                              Paste a {activeProductTab === 'services' ? 'service' : 'product'} URL to automatically extract details
                            </CardDescription>
                          </CardHeader>
                          <CardContent className="space-y-4">
                            <div>
                              <Label htmlFor="product-url">{activeProductTab === 'services' ? 'Service' : 'Product'} URL</Label>
                              <Input
                                id="product-url"
                                type="url"
                                value={productUrl}
                            ...
                            >
                              {isExtracting ? 'Extracting...' : `Extract ${activeProductTab === 'services' ? 'Service' : 'Product'} Details`}
                            </Button>
                          </CardContent>
                        </Card>
                          ...
                          <Card>
                            <CardHeader>
                              <CardTitle className="text-lg text-navy dark:text-blue-400">
                                ✍️ Manual {activeProductTab === 'services' ? 'Service' : 'Product'} Entry
                              </CardTitle>
                              <CardDescription>
                                Enter {activeProductTab === 'services' ? 'service' : 'product'} details manually for {categoryInfo.title}
                              </CardDescription>
                            </CardHeader>
                            <CardContent>
                              <div className="space-y-4">
                                <div className="grid md:grid-cols-2 gap-4">
                                  <div>
                                    <Label htmlFor="manual-name">{activeProductTab === 'services' ? 'Service' : 'Product'} Name *</Label>
                                    <Input
                                      id="manual-name"
                                      value={manualProduct.name}
                                      onChange={(e) => setManualProduct({...manualProduct, name: e.target.value})}
                                      placeholder={`Enter ${activeProductTab === 'services' ? 'service' : 'product'} name`}
                                      required
                                    />
                                  </div>
                                  <div>
                                ...
                                {activeProductTab === 'services' && (
                                  <div className="space-y-4 border-t pt-4">
                                    <h4 className="text-lg font-semibold text-purple-600">Service Pricing Options</h4>
                                    ...
                                    <div className="flex items-center space-x-2">
                                      <input
                                        ...
                                        className="rounded"
                                      />
                                      <Label htmlFor="manual-free">Free Service</Label>
                                    </div>
                                ...
                                <div className="flex items-center space-x-4">
                                  <div className="flex items-center space-x-2">
                                    <input
                                      ...
                                      className="rounded"
                                    />
                                    <Label htmlFor="manual-featured">Featured {activeProductTab === 'services' ? 'Service' : 'Product'}</Label>
                                  </div>
                                  <div className="flex items-center space-x-2">
                                    <input
                                      type="checkbox"
                                      ...
                                      className="rounded"
                                    />
                                    <Label htmlFor="manual-timer">Enable Timer</Label>
                                  </div>
                                  {activeProductTab === 'services' && (
                                    <div className="flex items-center space-x-2">
                                      <input
                                ...
                                <div className="border-t pt-4">
                                  ...
                                  {customFields.length === 0 && (
                                    <div className="text-center py-4 text-gray-500 text-sm">
                                      ...
                                      No custom fields added. Click "Add Custom Field" to add {activeProductTab === 'services' ? 'service' : 'product'}-specific information.
                                    </div>
                                  )}
                                </div>
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          ...
                          {(product as any).isFree || ((product as any).pricingType === 'free') ? (
                            ...
                            <div className="flex items-center space-x-2">
                              <span className="text-2xl font-bold text-navy dark:text-blue-400">₹{(product as any).monthlyPrice}/month</span>
                              {product.originalPrice && (
                                <span className="text-gray-400 line-through text-sm">₹{product.originalPrice}/month</span>
                              )}
                            </div>
                          ) : (product as any).yearlyPrice && (product as any).yearlyPrice !== '0' ? (
                            <div className="flex items-center space-x-2">
                              <span className="text-2xl font-bold text-navy dark:text-blue-400">₹{(product as any).yearlyPrice}/year</span>
                              {product.originalPrice && (
                                <span className="text-gray-400 line-through text-sm">₹{product.originalPrice}/year</span>
                              )}
                            </div>
                          ) : (
                            <div className="flex items-center space-x-2">
                              ...
                              {(product as any).pricingType === 'monthly' ? (
                                <span className="text-2xl font-bold text-navy dark:text-blue-400">₹{product.price}/month</span>
                              ) : (product as any).pricingType === 'yearly' ? (
                                <span className="text-2xl font-bold text-navy dark:text-blue-400">₹{product.price}/year</span>
                              ) : (
