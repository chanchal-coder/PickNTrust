              ...
              >
                ...
                <div className="p-4 bg-white dark:bg-gray-800 text-gray-900 dark:text-white space-y-2 relative">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {(service as any).isFree || ((service as any).pricingType === 'free') || service.price === '0' ? (
                      <span className="text-lg font-bold text-green-600 dark:text-green-400">FREE</span>
                    ) : (service as any).priceDescription ? (
                      <span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">{(service as any).priceDescription}</span>
                    ) : (service as any).monthlyPrice && (service as any).monthlyPrice !== '0' ? (
                      <div className="flex items-center space-x-2">
                        <span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">₹{(service as any).monthlyPrice}/month</span>
                        {service.originalPrice && (
                          <span className="text-gray-500 line-through text-sm">₹{service.originalPrice}/month</span>
                        )}
                      </div>
                    ) : (service as any).yearlyPrice && (service as any).yearlyPrice !== '0' ? (
                      <div className="flex items-center space-x-2">
                        <span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">₹{(service as any).yearlyPrice}/year</span>
                        {service.originalPrice && (
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        {service.originalPrice && (
                          <span className="text-gray-500 line-through text-sm">
                            ₹{service.originalPrice}
                            {(service as any).pricingType === 'monthly' ? '/month' : (service as any).pricingType === 'yearly' ? '/year' : ''}
                          </span>
                        )}
                      </div>
                    )}
            ...
            {displayServices.map((service: Product, index: number) => (
              ...
              >
                ...
                <div className="p-3">
                  ...
                  <button 
                    onClick={() => handleAffiliateClick(service)}
                    className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold py-2 px-3 rounded-lg transition-all duration-300 text-sm mb-2"
                  >
                    Pick Now
                  </button>
                  ...
                  <div className="flex gap-2 justify-end">
                    {/* Share Button - Rounded Blue Icon */}
                    <div className="relative">
                      <button
                        onClick={() => setShowShareMenu(prev => ({...prev, [service.id]: !prev[service.id]}))}
