  ...
  const addProductMutation = useMutation({
    mutationFn: async (productData: any) => {
      ...
      const payload = {
        ...
        monthlyPrice: cleanMonthlyPrice || null,
        yearlyPrice: cleanYearlyPrice || null,
  ...
  const handleAddProduct = (e: React.FormEvent) => {
    ...
    if (!newProduct.name.trim()) {
      toast({
        ...
        description: `${activeTab === 'services' ? 'Service' : 'Product'} name is required`,
        variant: 'destructive',
      });
      return;
    }
        ...
        <CardContent>
          {!isAddingProduct ? (
            ...
            <form onSubmit={handleAddProduct} className="space-y-4">
              ...
              {/* Pricing Section - Different for Products vs Services */}
              {activeTab === 'services' ? (
                <div className="space-y-4">
                  <h3 className="text-lg font-semibold text-purple-300">Service Pricing</h3>
                  ...
                  {!newProduct.isFree && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      {(newProduct.pricingType === 'monthly' || newProduct.pricingType === 'custom') && (
                        <div>
                          <label className="block text-sm font-medium mb-2 text-blue-300">Monthly Price (₹)</label>
                          <input
                            type="text"
                      ...
                      {(newProduct.pricingType === 'yearly' || newProduct.pricingType === 'custom') && (
                        <div>
                          <label className="block text-sm font-medium mb-2 text-blue-300">Yearly Price (₹)</label>
                          <input
                            type="text"
              ...
              {activeTab === 'services' && (
                <div className="bg-purple-900/30 border border-purple-600/50 rounded-lg p-4">
                  ...
                  <p className="text-purple-200 text-xs">
                    You're adding a service product for the Cards, Apps & Services section. Use the description field to include service-specific details like provider, features, eligibility, and terms.
                  </p>
                </div>
              )}
