...
export class CanvaService {
  ...
  private generateSmartCaption(data: CanvaDesignData): string {
    ...
    let caption = `🛍️ Amazing ${category || 'Deal'} Alert! \n\n`;
    caption += `✨ ${title}\n`;
    ...
    if (price) {
      if (price === '0') {
        ...
        if (originalPrice && originalPrice !== price) {
          ...
          caption += ` (was ₹${originalPrice}) - ${discount}% OFF! 🔥\n`;
        } else {
          caption += `\n`;
        }
      }
  ...
  async autoPostToAllPlatforms(
    ...
    console.log(`🤖 AUTO-POSTING: Starting automatic posting to ${enabledPlatforms.length} platforms...`);
    ...
    for (const platform of enabledPlatforms) {
      try {
        ...
        console.log(`❌ ${platform}: Exception - ${error}`);
      }
    }
    ...
    if (results.success.length === enabledPlatforms.length) {
      results.message = `🎉 SUCCESS: Posted to all ${results.success.length} platforms!`;
    } else if (results.success.length > 0) {
      results.message = `⚠️ PARTIAL: Posted to ${results.success.length}/${results.total} platforms. ${results.failed.length} failed.`;
    } else {
      results.message = `❌ FAILED: Could not post to any platforms. Check API credentials.`;
    }
    
    console.log(`🤖 AUTO-POSTING COMPLETE: ${results.message}`);
    ...
    try {
      ...
      const response = await fetch(`https://api.telegram.org/bot${botToken}/sendPhoto`, {
        ...
        body: JSON.stringify({
          chat_id: channelId,
          photo: imageUrl,
          caption: `${caption}\n\n📱 Join @pickntrust for more amazing deals!`,
          parse_mode: 'HTML',
          reply_markup: {
            inline_keyboard: [[
              {
      ...
      const communityResponse = await fetch(`https://www.googleapis.com/youtube/v3/activities?part=snippet&key=${youtubeApiKey}`, {
        ...
        body: JSON.stringify({
          snippet: {
            channelId: channelId,
            type: 'channelItem',
            description: `${caption}\n\n🔗 ${websiteUrl}\n\n📱 Subscribe for more amazing deals!\n\n#PicknTrust #Deals #Shopping`,
            thumbnails: {
      ...
      await fetch(`https://www.googleapis.com/youtube/v3/commentThreads?part=snippet&key=${youtubeApiKey}`, {
        ...
        body: JSON.stringify({
          snippet: {
            ...
            topLevelComment: {
              snippet: {
                textOriginal: `🛍️ Get this deal: ${websiteUrl}\n\n📱 Visit PicknTrust.com for more amazing deals!`
              }
            }
          }
        })
  ...
  async executeFullAutomation(contentData: ContentAutoPostData, enabledPlatforms: string[]): Promise<AutoPostingResults> {
    try {
      console.log(`🤖 STARTING FULL AUTOMATION for ${contentData.contentType}: ${contentData.title}`);
      
      let designId: string | null = null;
      let useCanvaDesign = true;
      
      // Step 1: Try to create Canva design (with fallback)
      console.log('🎨 Step 1: Attempting Canva design creation...');
      try {
        ...
        console.warn('⚠️ Canva design failed, using fallback approach:', canvaError);
        useCanvaDesign = false;
        // Use the original content image as fallback
        designId = contentData.imageUrl || 'fallback-image';
      }
      ...
      console.log(`📱 Step 3: Auto-posting to social platforms (${useCanvaDesign ? 'with Canva design' : 'with fallback image'})...`);
      ...
      console.log('📊 Step 4: Automation complete!');
      console.log(`✅ Success: ${results.success.length} platforms`);
      ...
      if (!useCanvaDesign && results.success.length > 0) {
        results.message = `⚠️ PARTIAL SUCCESS: Posted to ${results.success.length}/${results.total} platforms (Canva unavailable, used fallback images)`;
      }
      ...
      console.log('🔄 Attempting emergency fallback posting...');
      try {
        const { caption, hashtags } = await this.generateCaptionAndHashtags({
          title: contentData.title,
          description: contentData.description,
        ...
        if (emergencyResults.success.length > 0) {
          emergencyResults.message = `🚨 EMERGENCY SUCCESS: Posted to ${emergencyResults.success.length}/${emergencyResults.total} platforms (no design, text-only posts)`;
