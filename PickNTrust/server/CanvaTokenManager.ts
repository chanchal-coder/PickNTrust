...
export class CanvaTokenManager {
  ...
  private async refreshAccessToken(refreshToken: string): Promise<string> {
    try {
      console.log('🔄 Refreshing Canva access token...');
      ...
      if (!response.ok) {
        console.error('❌ Canva token refresh failed:', responseText);
        
        // Handle specific error cases
        if (responseText.includes('invalid_grant') || responseText.includes('lineage')) {
          console.error('🚨 Refresh token lineage revoked. Manual re-auth required.');
      ...
      console.log('✅ Canva access token refreshed successfully');
  ...
  private loadTokenData(): CanvaTokenData | null {
    try {
      if (!fs.existsSync(this.tokenStorePath)) {
        ...
        if (envRefreshToken) {
          console.log('📦 Migrating Canva token from environment variables...');
      ...
      console.error('❌ Error loading Canva token data:', error);
      return null;
    }
  }
  ...
  private saveTokenData(tokenData: CanvaTokenData): void {
    try {
      ...
      console.log('💾 Canva tokens saved to persistent storage');
    } catch (error) {
      console.error('❌ Error saving Canva token data:', error);
      throw error;
    }
  }
  ...
  async initializeWithRefreshToken(refreshToken: string): Promise<void> {
    console.log('🚀 Initializing Canva token manager with refresh token...');
    ...
    try {
      ...
      console.log('✅ Canva token manager initialized successfully');
    } catch (error) {
      console.error('❌ Failed to initialize with provided refresh token:', error);
      this.markTokensInvalid();
      throw error;
    }
  }
