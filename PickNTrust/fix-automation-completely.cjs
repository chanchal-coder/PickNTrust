  ...
  try {
    ...
    db.exec(`
      CREATE TABLE IF NOT EXISTS canva_settings (
        ...
        default_title TEXT DEFAULT '🛍️ Amazing {category} Deal: {title}',
        default_caption TEXT DEFAULT '🛍️ Amazing {category} Alert! ✨ {title} 💰 Price: ₹{price} 🔗 Get the best deals at PickNTrust!',
    ...
    if (existingSettings.count === 0) {
      ...
      db.prepare(`
        UPDATE canva_settings SET 
          ...
          default_title = COALESCE(default_title, '🛍️ Amazing {category} Deal: {title}'),
      ...
      console.log('✅ Updated existing Canva settings');
    }
    ...
    try {
      ...
      console.log('⚠️ Template already exists or error:', error.message);
    }
    ...
    if (envUpdated) {
      ...
      console.log('✅ Updated .env file with Canva variables');
    } else {
      console.log('✅ .env file already has required variables');
    }
    ...
    if (fs.existsSync('server/routes.ts')) {
      ...
      console.log(`✅ Product automation trigger: ${hasProductTrigger ? 'Present' : 'Missing'}`);
      console.log(`✅ Blog automation trigger: ${hasBlogTrigger ? 'Present' : 'Missing'}`);
      console.log(`✅ Video automation trigger: ${hasVideoTrigger ? 'Present' : 'Missing'}`);
    }
    ...
    try {
      ...
      console.log('✅ Can read canva_settings');
      
      const postCount = db.prepare('SELECT COUNT(*) as count FROM canva_posts').get();
      console.log(`✅ Can read canva_posts (${postCount.count} records)`);
      
      const templateCount = db.prepare('SELECT COUNT(*) as count FROM canva_templates').get();
      console.log(`✅ Can read canva_templates (${templateCount.count} records)`);
    } catch (error) {
    ...
    console.log('✅ Canva automation enabled');
    console.log('✅ Default settings configured');
    console.log('✅ Environment variables set');
    console.log('✅ Automation triggers verified');
