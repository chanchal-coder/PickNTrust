...
try {
  ...
  if (!currentData) {
    ...
    console.log('✅ Default settings inserted with ID:', result.lastInsertRowid);
    
  } else {
    console.log('📝 Existing settings found. Updating default values...');
    ...
    console.log('✅ Settings updated. Changes:', result.changes);
  }
  ...
  db.close();
  console.log('\n🎉 Default values setup complete!');
  console.log('🚀 Your Canva automation is now ready with proper default templates!');
  
} catch (error) {
  console.error('❌ Error setting up default values:', error.message);
