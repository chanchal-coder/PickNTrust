...
console.log('🔍 Checking products table schema and data...');
...
try {
  ...
  if (problematicColumns.length > 0) {
    problematicColumns.forEach(col => {
      console.log(`  ⚠️  ${col.name}: ${col.type} (might cause validation issues)`);
    });
  } else {
    console.log('  ✅ No obvious data type issues found');
  }
  ...
  try {
    ...
    console.log(`  ✅ Test insertion successful! New product ID: ${result.lastInsertRowid}`);
  ...
  console.error('❌ Error checking products schema:', error.message);
} finally {
  db.close();
