...
try {
  ...
  if (fs.existsSync(envPath)) {
    ...
    if (existingContent.includes('CANVA_CLIENT_ID')) {
      console.log('✅ Canva credentials already exist in .env file');
    } else {
      // Append Canva credentials to existing .env
      fs.appendFileSync(envPath, canvaCredentials);
      console.log('✅ Canva credentials template added to existing .env file');
    }
  } else {
    // Create new .env file with Canva credentials
    fs.writeFileSync(envPath, canvaCredentials.trim());
    console.log('✅ Created new .env file with Canva credentials template');
  }

  console.log('\n🎉 Canva API setup complete!');
  console.log('\nCredentials template added to .env file');
  console.log('⚠️  IMPORTANT: Replace the placeholder values with your actual Canva API credentials');
  ...
  console.error('❌ Error setting up Canva credentials:', error.message);
  console.log('\n📝 Manual setup:');
