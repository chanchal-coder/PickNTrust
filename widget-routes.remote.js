...
const verifyAdminAccess = (req, res, next) => {
    ...
    const headerPwd = (req.headers['x-admin-password'] || req.headers['X-Admin-Password']);
    const bodyPwd = (req.body && typeof req.body === 'object') ? req.body.password : undefined;
...
// Get all widgets
router.get('/api/admin/widgets', verifyAdminAccess, async (req, res) => {
    try {
...
router.get('/api/widgets/:page/:position', async (req, res) => {
    try {
...
router.get('/api/widgets/:page', async (req, res) => {
    try {
...
// Create a new widget
router.post('/api/admin/widgets', verifyAdminAccess, async (req, res) => {
    try {
    ...
    `).run(validatedData.name, validatedData.description || null, validatedData.body || null, validatedData.code, validatedData.targetPage, validatedData.position, (validatedData.isActive !== false) ? 1 : 0, // Convert boolean to integer
        validatedData.displayOrder || 0, validatedData.maxWidth || null, validatedData.customCss || null, (validatedData.showOnMobile !== false) ? 1 : 0, // Convert boolean to integer
...
// Update a widget
router.put('/api/admin/widgets/:id', verifyAdminAccess, async (req, res) => {
    try {
        const { id } = req.params;
        const validatedData = insertWidgetSchema.partial().parse(req.body);
...
router.patch('/api/admin/widgets/:id/toggle', verifyAdminAccess, async (req, res) => {
    try {
        const { id } = req.params;
        // First get the current status
        const currentWidget = sqliteDb.prepare('SELECT is_active FROM widgets WHERE id = ?').get(parseInt(id));
        if (!currentWidget) {
            return res.status(404).json({ error: 'Widget not found' });
        }
...
// Delete a widget
router.delete('/api/admin/widgets/:id', verifyAdminAccess, async (req, res) => {
    try {
        const { id } = req.params;
...
router.put('/api/admin/widgets/reorder', verifyAdminAccess, async (req, res) => {
    try {
...
router.get('/api/admin/widgets/stats', verifyAdminAccess, async (req, res) => {
    try {
        const totalWidgets = sqliteDb.prepare('SELECT * FROM widgets').all();
        const activeWidgets = totalWidgets.filter((w) => w.is_active);
        const statsByPage = totalWidgets.reduce((acc, widget) => {
        ...
        const statsByPosition = totalWidgets.reduce((acc, widget) => {
