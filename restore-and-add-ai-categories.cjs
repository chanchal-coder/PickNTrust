...
try {
  ...
  console.log('\n🔄 Resetting incorrectly marked categories:');
  ...
  categoriesToReset.forEach(categoryName => {
    ...
    if (result.changes > 0) {
      console.log(`  ✅ Reset "${categoryName}" (no longer AI Apps category)`);
      resetCount++;
    }
  });
  ...
  aiCategories.forEach(category => {
    ...
    if (result.changes > 0) {
      console.log(`  ✅ Added "${category.name}" as AI Apps category`);
      addedCount++;
    } else {
  ...
  console.log(`\n🎉 Successfully reset ${resetCount} categories and added ${addedCount} new AI categories!`);
  console.log('\n💡 The AI Apps dropdown now shows proper AI-specific categories.');
