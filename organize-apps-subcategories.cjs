...
try {
  ...
  console.log(`✅ Found main category: ${mainAppsCategory.id}. ${mainAppsCategory.name}`);
  ...
  console.log(`\n🔄 Converting ${categoriesToMakeSubcategories.length} categories to subcategories...`);
  ...
  categoriesToMakeSubcategories.forEach(categoryName => {
    ...
    if (result.changes > 0) {
      updatedCount++;
      console.log(`  ✅ ${categoryName} → subcategory of "Apps & AI Apps"`);
    } else {
      console.log(`  ⚠️  ${categoryName} not found or already a subcategory`);
    }
  });
  
  console.log(`\n📊 Summary:`);
  console.log(`  ✅ Categories converted to subcategories: ${updatedCount}`);
  ...
  console.log(`  📁 ${mainAppsCategory.name} (${subcategories.length} subcategories):`);
  subcategories.forEach(sub => {
    console.log(`    └─ ${sub.id}. ${sub.name}`);
  });
  
  console.log('\n✅ Apps categories reorganization completed!');
  console.log('\n📝 Next steps:');
  ...
  console.error('❌ Error organizing categories:', error.message);
  process.exit(1);
}