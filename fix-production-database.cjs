...
try {
  ...
  try {
    ...
    if (!tableExists) {
      ...
      console.log('   ✅ Created video_content indexes');
      
    } else {
      console.log('   ✅ video_content table already exists');
    }
  } catch (error) {
    console.log(`   ❌ Failed to create video_content table: ${error.message}`);
  }
  ...
  if (timerStartTimeCol && timerStartTimeCol.type === 'TEXT') {
    ...
    try {
      ...
      console.log('   ✅ Converted timer_start_time to INTEGER');
    } catch (error) {
      console.log(`   ❌ Failed to convert timer_start_time: ${error.message}`);
    }
  } else {
    console.log('   ✅ timer_start_time column type is correct');
  }
  ...
  try {
    ...
    console.log(`   ✅ Products table test successful (ID: ${testProduct.lastInsertRowid})`);
  ...
  try {
    ...
    console.log(`   ✅ Video content table test successful (ID: ${testVideo.lastInsertRowid})`);
    
    // Clean up test video
    db.prepare('DELETE FROM video_content WHERE id = ?').run(testVideo.lastInsertRowid);
    
  } catch (error) {
    console.log(`   ❌ Video content table test failed: ${error.message}`);
  }
  ...
  console.log('\n✅ Production database schema fix completed!');
