...
try {
  ...
  generalAppCategories.forEach(categoryName => {
    ...
    if (result.changes > 0) {
      console.log(`  ✅ Updated existing "${categoryName}" to include AI Apps`);
      updatedCount++;
    } else {
      toCreateCount++;
    }
  ...
  newCategories.forEach(category => {
    ...
    if (result.changes > 0) {
      console.log(`  ✅ Added "${category.name}" as new AI Apps category`);
      addedCount++;
    } else {
  ...
  console.log('\n🤖 Final AI Apps categories (Apps + AI Apps):');
  ...
  console.log(`\n🎉 Successfully updated ${updatedCount} existing categories and added ${addedCount} new categories!`);
  console.log(`\n📊 Total AI Apps categories: ${finalAIApps.length}`);
  console.log('\n💡 The AI Apps dropdown now includes both general app categories and AI-specific categories.');
