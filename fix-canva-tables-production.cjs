...
try {
  ...
  console.log('✅ Canva tables found:');
  ...
  if (settings) {
    console.log('✅ Default Canva settings created:');
  ...
  console.log('\n🎉 SUCCESS: All Canva automation tables created successfully!');
  console.log('📱 You can now use the Automation Management panel in the admin area.');

} catch (error) {
  console.error('❌ ERROR creating Canva tables:', error);
  process.exit(1);
} finally {
  db.close();
}
