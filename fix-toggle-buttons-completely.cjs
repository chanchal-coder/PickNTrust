...
async function fixToggleButtonsCompletely() {
  try {
    ...
    if (existingSettings.count === 0) {
      ...
      console.log('✅ Created default settings with automation OFF');
    } else {
      ...
      console.log('✅ Reset existing settings to known good state');
    }
    ...
    console.log('✅ Enable test:', result.is_enabled === 1 ? 'SUCCESS' : 'FAILED');
    ...
    console.log('✅ Disable test:', result.is_enabled === 0 ? 'SUCCESS' : 'FAILED');
    ...
    console.log('✅ Caption disable test:', result.auto_generate_captions === 0 ? 'SUCCESS' : 'FAILED');
    ...
    result = db.prepare('SELECT auto_generate_captions FROM canva_settings WHERE id = 1').get();
    console.log('✅ Caption enable test:', result.auto_generate_captions === 1 ? 'SUCCESS' : 'FAILED');
    ...
    console.log('✅ Hashtag disable test:', result.auto_generate_hashtags === 0 ? 'SUCCESS' : 'FAILED');
    ...
    result = db.prepare('SELECT auto_generate_hashtags FROM canva_settings WHERE id = 1').get();
    console.log('✅ Hashtag enable test:', result.auto_generate_hashtags === 1 ? 'SUCCESS' : 'FAILED');
...
`;
    ...
    console.log('✅ Created API test script: test-canva-api.mjs');
...
## Expected Behavior
- Database operations: ✅ Working (tested)
- API endpoints: ✅ Working (in routes.ts)
- Frontend state: ❌ Likely the issue
...
`;
    ...
    console.log('✅ Created debugging guide: CANVA_TOGGLE_DEBUG_GUIDE.md');
