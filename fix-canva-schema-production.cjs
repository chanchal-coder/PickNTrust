...
try {
  ...
  console.log('Search Checking current canva_settings table structure...');
  ...
  for (const column of requiredColumns) {
    if (!existingColumns.includes(column.name)) {
      ...
      try {
        ...
        console.error(`Error Error adding column ${column.name}:`, error.message);
      }
    }
  }
  
  if (columnsAdded === 0) {
    console.log('Success All required columns already exist');
  } else {
    console.log(`Success Added ${columnsAdded} missing columns`);
  }
  
  // Verify the table structure after migration
  console.log('Search Verifying updated table structure...');
  ...
  console.log(`Stats Current settings count: ${settingsCount.count}`);
  ...
  console.log('Celebration Canva schema migration completed successfully!');
  console.log('');
  console.log('📋 Summary:');
  ...
  console.log('');
  console.log('Launch You can now restart your PM2 process:');
