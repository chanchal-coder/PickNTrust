...
console.log('🔍 CHECKING CANVA CREDENTIALS STATUS...\n');
...
if (fs.existsSync(envPath)) {
  ...
  console.log(`${hasCanvaClientId ? '✅' : '❌'} CANVA_CLIENT_ID: ${hasCanvaClientId ? 'CONFIGURED' : 'MISSING OR PLACEHOLDER'}`);
  console.log(`${hasCanvaSecret ? '✅' : '❌'} CANVA_CLIENT_SECRET: ${hasCanvaSecret ? 'CONFIGURED' : 'MISSING OR PLACEHOLDER'}`);
  
  if (hasCanvaClientId && hasCanvaSecret) {
    console.log('\n🎉 Canva credentials are properly configured!');
  } else {
    console.log('\n⚠️  Canva credentials need to be configured.');
