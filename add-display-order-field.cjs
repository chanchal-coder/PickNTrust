...
console.log('Refresh Adding display order field to categories...');
...
try {
  ...
  if (!existingColumns.includes('display_order')) {
    console.log('Blog Adding display_order column...');
    sqlite.exec(`ALTER TABLE categories ADD COLUMN display_order INTEGER DEFAULT 0;`);
    console.log('Success display_order column added successfully');
  } else {
    console.log('Success display_order column already exists');
  }

  // Set initial display order based on current category order (by id)
  console.log('Refresh Setting initial display order values...');
  ...
  categories.forEach((category, index) => {
    ...
    console.log(`  Success ${category.name}: display_order = ${displayOrder}`);
  });

  console.log('Success Initial display order values set successfully');
  ...
  console.log('Stats Categories with display order:');
  ...
  sqlite.close();
  console.log('Celebration Display order field migration completed successfully!');
